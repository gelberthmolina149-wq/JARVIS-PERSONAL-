...
class JarvisLive:
    ...
    async def _run_openrouter_loop(self):
        """Fallback mode: OpenRouter for AI + edge-tts for voice when Gemini Live is unavailable."""
        import urllib.request as _ur
        import json as _json

        self._loop = asyncio.get_event_loop()
        self.ui.write_log("SYS: Modo OpenRouter activo. Escribe en el chat o usa el microfono.")
        self.ui.set_state("LISTENING")
        ...
        def _ask_openrouter(text: str) -> str:
            try:
                ...
                system_prompt = PROMPT_PATH.read_text(encoding="utf-8") if PROMPT_PATH.exists() else "Eres JARVIS, asistente IA."
                payload = _json.dumps({
                ...
                return f"Error al conectar con OpenRouter: {e}"
        ...
        async def _speak_edge(text: str):
            try:
                ...
                self.set_speaking(True)
                self.ui.set_state("SPEAKING")
                cfg_tts = _json.loads(API_CONFIG_PATH.read_text(encoding="utf-8"))
                voice = cfg_tts.get("tts_voice", "es-ES-AlvaroNeural")
                rate  = cfg_tts.get("tts_rate",  "-12%")
                pitch = cfg_tts.get("tts_pitch", "-20Hz")
                communicate = edge_tts.Communicate(text, voice, rate=rate, pitch=pitch)
                tmp = tempfile.NamedTemporaryFile(suffix=".mp3", delete=False)
                tmp.close()
                await communicate.save(tmp.name)
                # Reproducir con Windows Media Player en background
                proc = subprocess.Popen(
                    ["powershell", "-c", f"(New-Object Media.SoundPlayer).Play(); Add-Type -AssemblyName presentationCore; $mp = [System.Windows.Media.MediaPlayer]::new(); $mp.Open([uri]'{tmp.name}'); $mp.Play(); Start-Sleep 10"],
                    stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
                )
                # Esperar duración aproximada (100 chars ≈ 5s)
                await asyncio.sleep(max(3, len(text) / 20))
                proc.terminate()
        ...
        def _on_text_fallback(text: str):
            if not text.strip():
                return
            ...
            def _run_in_thread():
                ...
                self.ui.set_state("THINKING")
                self.ui.clear_jarvis_response()
                response = _ask_openrouter(text)
                # Display response on screen
                self.ui.stream_jarvis_chunk(response)
                # Speak response
                asyncio.run_coroutine_threadsafe(_speak_edge(response), self._loop)

            import threading
            threading.Thread(target=_run_in_thread, daemon=True).start()

        self.ui.on_text_command = _on_text_fallback
        # Keep alive — wait forever until app closes
        while True:
            await asyncio.sleep(1)

    async def run(self):
        # Skip Gemini Live entirely if user configured OpenRouter as primary provider
        try:
            import json as _j
            _cfg = _j.loads(API_CONFIG_PATH.read_text(encoding="utf-8"))
            if _cfg.get("ai_provider", "gemini") == "openrouter":
                print("[JARVIS] Modo OpenRouter configurado. Saltando Gemini Live...")
                self.ui.write_log("SYS: Modo OpenRouter+TTS activo.")
                self.ui.set_state("LISTENING")
                await self._run_openrouter_loop()
                return
        except Exception:
            pass
        ...
        while True:
            try:
                ...
                for exc in exceptions:
                    ...
                    elif "1008" in msg or "policy violation" in msg.lower() or "not found for API version" in msg or "UNAUTHENTICATED" in msg or "invalid authentication" in msg.lower():
                        print(f"[JARVIS] ⚠️ Modelo no disponible en esta versión de API: {msg[:120]}")
                        consecutive_fails += 1
                        if consecutive_fails >= 3:
                            print("[JARVIS] Cambiando a modo OpenRouter fallback...")
                            self.ui.write_log("SYS: Gemini Live no disponible. Cambiando a modo OpenRouter+TTS...")
                            self.ui.set_state("LISTENING")
                            await self._run_openrouter_loop()
                            return
